...
// It also returns parse failures so callers can warn about malformed records.
func HTTPSLookup(domain string, timeout time.Duration) ([]Record, []ParseFailure, error) {
	if err := validateDomain(domain); err != nil {
		return nil, nil, fmt.Errorf("https lookup: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, nil, fmt.Errorf("https lookup: status %d", resp.StatusCode)
	}

	body, err := io.ReadAll(io.LimitReader(resp.Body, maxWellKnownSize))
	if err != nil {
		return nil, nil, fmt.Errorf("https lookup: read body: %w", err)
	}

	records, failures := ParseRecordsAndFailures(string(body))
	if len(records) == 0 {
		return nil, failures, fmt.Errorf("https lookup: no valid records at %s", url)
	}
